"""
Used to replace **********************YOUR ENVIRONMENT**********************/lib/python3.10/site-packages/transformer_lens/HookedTransformer.py.
 """
import logging
...
class HookedTransformer(HookedRootModule):
    ...
    def forward(
        ...
        with utils.LocallyOverridenDefaults(
            ...
            for i, block in enumerate(transformer_block_list):  # type: ignore
                ...
                quantized_weights = self.quantize_block_weights(block)

                residual_quantized, scale, zero_point = self.quantize_tensor(residual)

                residual = self.dequantize_tensor(residual_quantized, scale, zero_point)
    ...
    def dequantize_tensor(self, quantized, scale, zero_point):
        ...
        if scale == 0: 
            raise ValueError("Scale cannot be zero during dequantization.")
