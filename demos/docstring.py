...
import sys
sys.path.append('/root/autodl-tmp/edge-attribution-patching')
...
class Prompt:
    def __init__(self, clean_prompt: str, corrupt_prompt: Union[str, Dict[str, str]], correct_answers: List[str], wrong_answers: List[str]):
...
def docstring_induction_prompt_generator(
    ...
    *,
    ...
    # Generate corrupt prompts (example for one)
    ...
    return Prompt(
        clean_prompt=clean_prompt,
        corrupt_prompt=random_doc_prompt,
        correct_answers=correct_answers,
        wrong_answers=wrong_answers
    )
...
def get_docstring_data(model: HookedTransformer) -> BatchedPrompts:
    """生成文档字符串任务的clean/corr数据"""
    prompts = [
        docstring_induction_prompt_generator(
            style=STYLE,
            n_matching_args=N_MATCHING_ARGS,
            n_def_prefix_args=N_DEF_PREFIX_ARGS,
            n_def_suffix_args=N_DEF_SUFFIX_ARGS,
            n_doc_prefix_args=N_DOC_PREFIX_ARGS,
            seed=42+i
        ) for i in range(N_EXAMPLES)
    ]
    return BatchedPrompts(prompts, model)
...
def doc_metric(
    ...
    """文档字符串任务评估指标"""
    correct_probs = batched_prompts.correct_prob(logits)
    return correct_probs.mean() if not per_prompt else correct_probs

if __name__ == "__main__":
    # 初始化模型和数据
    model = HookedTransformer.from_pretrained("gpt2-small", device=DEVICE)
    data = get_docstring_data(model)
    
    # 构建clean/corr数据
    clean_data = data.clean_tokens
    corr_data = data.corrupt_tokens["random_def_doc"]  # 选择特定腐败类型
    
    # 定义指标函数
    metric_fn = partial(doc_metric, batched_prompts=data)
    
    # 初始化EAP分析器
    graph = EAP(
        model=model,
        clean_input=clean_data,
        corrupt_input=corr_data,
        metric=metric_fn,
    ...
    for from_edge, to_edge, score in top_edges:
        print(f"{from_edge:>30} -> [{score:.3f}] -> {to_edge}")